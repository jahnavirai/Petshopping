	...
	if(isset($_POST['place']))
	{
		include("db.php");
						...
						while($row1=$query21->fetch()):
							$query555=$con->prepare("select * from petdetails where pet_id='".$row1['pet_id']."'");
							$query555->execute();
							$row5=$query555->fetch();
							$h=$row5['pet_rate'];
							$t=$row1['qty'];
							$v=$h*$t;
							$query211=$con->prepare("insert into order_placed(pet_id,user_id,qty,total_price,order_date)values('".$row1['pet_id']."','".$row1['user_id']."','".$row1['qty']."',$v,Now())");
						$query211->execute();
